...
//
//	Game::getInstance().device->CreateBuffer(&constDesc, nullptr, &constBuf);
//	
//	InitPrimitives();
//	InitQuads();
//	InitMeshes();
...
//	if (errorCode) errorCode->Release();
//
//	Game::getInstance().device->CreateVertexShader(vertexPrimCompResult->GetBufferPointer(), vertexPrimCompResult->GetBufferSize(), nullptr, &vertexPrimShader);
//	Game::getInstance().device->CreatePixelShader(pixelPrimCompResult->GetBufferPointer(), pixelPrimCompResult->GetBufferSize(), nullptr, &pixelPrimShader);
//
...
//
//	Game::getInstance().device->CreateBuffer(&bufDesc, nullptr, &verticesPrim);
...
//
//	res = Game::getInstance().device->CreateRasterizerState(&rastDesc, &rastState);
//
//	
//	D3D11_DEPTH_STENCIL_DESC depthDesc = {};
//	depthDesc.DepthEnable = false;
//	
//	Game::getInstance().device->CreateDepthStencilState(&depthDesc, &depthState);
//
//	lines.reserve(100);
//}
//
...
//	Game::getInstance().device->CreateVertexShader(vertexQuadCompResult->GetBufferPointer(), vertexQuadCompResult->GetBufferSize(), nullptr, &vertexQuadShader);
//
//	if (errorCode)errorCode->Release();
//	
...
//	Game::getInstance().device->CreatePixelShader(pixelQuadCompResult->GetBufferPointer(), pixelQuadCompResult->GetBufferSize(), nullptr, &pixelQuadShader);
//
//	if (errorCode)errorCode->Release();
//
...
//	
//	Game::getInstance().device->CreateBuffer(&bufDesc, &subData, &quadBuf);
//
//	delete[] points;
//
...
//	}
//	Game::getInstance().device->CreateSamplerState(&samplDesc, &quadSampler);
//
...
//	}
//
//	Game::getInstance().device->CreateRasterizerState(&rastDesc, &quadRastState);
//}
//
//
...
//	Game::getInstance().device->CreateVertexShader(vertexMeshCompResult->GetBufferPointer(), vertexMeshCompResult->GetBufferSize(), nullptr, &vertexMeshShader);
//
//	if(errorCode) errorCode->Release();
//
...
//	Game::getInstance().device->CreatePixelShader(pixelMeshCompResult->GetBufferPointer(), pixelMeshCompResult->GetBufferSize(), nullptr, &pixelMeshShader);
//
//	if (errorCode) errorCode->Release();
//
...
//	}
//
//	Game::getInstance().device->CreateBuffer(&bufDesc, nullptr, &meshBuf);
//}
//
//
...
//
//	Game::getInstance().context->UpdateSubresource(constBuf, 0, nullptr, &mat, 0, 0);
//
//	Game::getInstance().context->OMSetDepthStencilState(depthState, 0);
//	Game::getInstance().context->RSSetState(rastState);
//
//	Game::getInstance().context->VSSetShader(vertexPrimShader, nullptr, 0);
//	Game::getInstance().context->PSSetShader(pixelPrimShader, nullptr, 0);
//
//	Game::getInstance().context->IASetInputLayout(layoutPrim);
//	Game::getInstance().context->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_LINELIST);
//
//	const UINT offset = 0;
//	Game::getInstance().context->IASetVertexBuffers(0, 1, &verticesPrim, &bufPrimStride, &offset);
//
//	Game::getInstance().context->VSSetConstantBuffers(0, 1, &constBuf);
//
//	Game::getInstance().context->Draw(pointsCount, 0);
//}
//
//
...
//
//	Game::getInstance().context->OMSetDepthStencilState(depthState, 0);
//	Game::getInstance().context->RSSetState(quadRastState);
//	
//	Game::getInstance().context->VSSetShader(vertexQuadShader, nullptr, 0);
//	Game::getInstance().context->PSSetShader(pixelQuadShader, nullptr, 0);
//
//	Game::getInstance().context->IASetInputLayout(quadLayout);
//	Game::getInstance().context->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLESTRIP);
//
//	const UINT offset = 0;
//	Game::getInstance().context->IASetVertexBuffers(0, 1, &quadBuf, &quadBindingStride, &offset);
//
//	Game::getInstance().context->VSSetConstantBuffers(0, 1, &constBuf);
//
//	for(auto& quad : quads) {
...
//		Game::getInstance().context->UpdateSubresource(constBuf, 0, nullptr, &mat, 0, 0);
//
//		Game::getInstance().context->PSSetShaderResources(0, 1, &quad.Srv);
//		Game::getInstance().context->PSSetSamplers(0, 1, &quadSampler);
//
//		Game::getInstance().context->Draw(4, 0);
//	}
//}
//
//
...
//	Game::getInstance().context->RSSetState(rastState);
//
//	Game::getInstance().context->VSSetShader(vertexMeshShader, nullptr, 0);
//	Game::getInstance().context->PSSetShader(pixelMeshShader, nullptr, 0);
//
//	Game::getInstance().context->IASetInputLayout(meshLayout);
//	Game::getInstance().context->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
//
//	Game::getInstance().context->VSSetConstantBuffers(1, 1, &meshBuf);
//	
//	const UINT offset = 0;
//	for(auto& mesh : meshes) {
//		Game::getInstance().context->IASetVertexBuffers(0, 1, &mesh.Mesh->VertexBuffer, &mesh.Mesh->Stride, &offset);
//		Game::getInstance().context->IASetIndexBuffer(mesh.Mesh->IndexBuffer, DXGI_FORMAT_R32_UINT, 0);
//
//		MeshConstData data = MeshConstData {
//			mesh.Transform * camera->GetViewMatrix()* camera->GetProjectionMatrix(),
//			mesh.Color
//		};
//
//		Game::getInstance().context->UpdateSubresource(meshBuf, 0, nullptr, &data, 0, 0);
//		
//		Game::getInstance().context->DrawIndexed(mesh.Mesh->IndexCount, 0, 0);
//	}
//}
//
//
...
//	Game::getInstance().context->Map(verticesPrim, 0, D3D11_MAP_WRITE_DISCARD, 0, &mapped);
//
//	memcpy(mapped.pData, lines.data(), lines.size() * VertexPositionColor::Stride);
//
//	Game::getInstance().context->Unmap(verticesPrim, 0);
//}
//
//
//DebugRenderSysImpl::~DebugRenderSysImpl()
...
//
//	Game::getInstance().context->ClearState();
//	game->RestoreTargets(); //restore back/depth buffer
//
...
//	};
//	
//	Game::getInstance().context->RSSetViewports(1, &viewport);
//
//	DrawPrimitives();
//	DrawQuads();
//	DrawMeshes();
