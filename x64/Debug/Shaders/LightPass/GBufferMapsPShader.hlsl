...
// DEPTHMAP777
// static const float4 DepthMapPos = float4(520.0f, 0.0f, 640.0f, 130.0f);
...
float4 PSMain(PS_IN input) : SV_Target
{
    ...
    // DEPTHMAP777
    /*
    else if (input.pos.x > DepthMapPos.x && input.pos.x < DepthMapPos.z
        && input.pos.y > DepthMapPos.y && input.pos.y < DepthMapPos.w)
    {
        float x = (input.pos.x - DepthMapPos.x) / (DepthMapPos.z - DepthMapPos.x);
        float y = (input.pos.y - DepthMapPos.y) / (DepthMapPos.w - DepthMapPos.y);
        float depthVal = DepthMap.Sample(Sampler, float2(x, y)).rrr;
        depthVal = log(0.5 * (depthVal + 1) / (1 - 0.5 * (depthVal + 1))) / 10;
        output = float4(depthVal.xxx, 1.0f);
    }
    */
